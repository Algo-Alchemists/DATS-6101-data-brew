...
```{r}
income_offer_counts <- final %>%
  group_by(income_group, offer_type) %>%
  summarize(count = n())

# Plotting the offer type preferences by income range
ggplot(income_offer_counts, aes(x = income_group, y = count, fill = offer_type)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  labs(title = 'Offer Type Preferences by Income Range',
       x = 'Income Range',
       y = 'Count') +
  theme_minimal() +
